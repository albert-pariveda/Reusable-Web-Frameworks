/*	DonutChart library file for creating a simple D3 donut chart.
 * 
 * 	Parameters: 	id		- jQuery element (e.g. #donutChart) that will contain the donut chart
 ...
 *  					var donutChart = new DonutChart("#donutChart", { width: 300, height: 450, title: "Percent of Visits"});
 *  
 *  				Then, after creating the object call donutChart.redrawChart(data) with your data array to draw the chart.
 *  				An example data array showing percentages of "Apples" vs "Oranges" would look like:
 *  					["Apples", 105],
 *  					["Oranges", 203] 
 *  
 *  				Subsequent calls to redrawChart() will delete the previous chart and draw a new one.
 *  
 *  				NOTE: If you wish to use the legend/explanation/labels that show up in the center of the donut chart be
 *  				 sure to either use or change the ID's listed below (e.g. #legend). They are not (yet) part of the config array.
 */
/*
 Based off of the Adobe Real-Time Dashboard Example (2014-03-27)
...
function DonutChart(id, config) {
  ...
  // Id's for legend/explanation/pagelabel elements
  this.legendId = config.legendId || "#legend";
  this.toggleLegendId = config.toggleLegendId || "#togglelegend";
  this.percentageId = config.percentageId || "#percentage";
  this.explanationId = config.explanationId || "#explanation";
  this.labelId = config.labelId || "#pagelabel";
  ...
  this.redrawChart = function(data) {
    ...
    //data.splice(this.colors.length);  // This will limit the sections to the number of colors
...
//    if($(window).width() <= 767){
//    	d3.select(id).attr("style", "margin: 0 auto");
//    }
    
    // Chart title
	this.vis.append("text")
		.attr("x", 0)             
		.attr("y", 50 - (this.height/2))
    ...
    var nodes = this.partition.nodes(json)
      .filter(function(d) {
        return (d.dx > 0.0000000001); // 0.005 radians = 0.29 degrees
    });
    ...
    var path = this.vis.data([json]).selectAll("path")
      ...
      .style("fill", function(d, i) {
          if (self.colors.length <= i-1) {
              self.colors.push(self.generateColor(i));
          }
    ...
    d3.select(id).on("mouseleave", this.mouseleave);
  ...
  this.drawLegend = function() {
    ...
    var legend = d3.select(this.legendId).append("svg:svg")
        .attr("width", li.w)
        .attr("height", this.legend.length * (li.h + li.s));
		
	// Ensure legend is sorted by value and not position in array
	this.legend = this.legend.sort(function(a,b) {
